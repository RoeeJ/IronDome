...
export class InstancedBuildingRenderer {
    ...
    sides.forEach((side, sideIndex) => {
      for (let row = 0; row < windowRows; row++) {
        for (let col = 0; col < side.cols; col++) {
          ...
          if (pool.length === 0) {
            debug.warn(
              `Window pool exhausted: ${isLit ? 'lit' : 'unlit'} pool empty at building ${building.id}`
            );
            continue;
          }
  ...
  updateWindowLighting(hours: number): void {
    ...
    this.buildings.forEach(building => {
      ...
      const targetLitPercentage = this.calculateBuildingLitPercentage(hours, buildingType, buildingHash);
   ...
   */
  private getBuildingLightingType(building: BuildingInstance, hash: number): 'residential' | 'office' | 'commercial' | 'industrial' {
