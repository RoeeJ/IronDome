...
export class WorldGeometryOptimizer {
  ...
  static optimizeStaticGeometry(
    ...
    const { mergeByMaterial = true, excludeNames = [], castShadows = false } = options;
  ...
  static analyzeScene(scene: THREE.Scene): {
    ...
    if (materials.size > 20) {
      recommendations.push(
        `Many unique materials (${materials.size}). Consider material atlasing.`
      );
    }

    if (duplicateGeometries > 50) {
      recommendations.push(
        `Many duplicate geometries (${duplicateGeometries}). Consider instancing.`
      );
    }
