...
export class StreetLightInstanceManager {
  ...
  private initializeInstancedMeshes(): void {
    ...
    const majorPoleGeometry = geometryFactory.getCylinder(
      majorPoleRadius,
      majorPoleRadius * 1.5,
      majorPoleHeight
    );
  ...
  animateLightAppear(index: number): void {
    ...
    for (const [_, instance] of this.instances) {
      if (instance.index === index) {
        ...
        if (instance.isMajor) {
          targetMeshes = [
            this.majorPoleInstancedMesh,
            this.majorFixtureInstancedMesh,
            this.majorBulbInstancedMesh,
          ];
        } else {
          targetMeshes = [
            this.poleInstancedMesh,
            this.fixtureInstancedMesh,
            this.bulbInstancedMesh,
          ];
        }
        break;
      }
