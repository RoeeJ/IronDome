...
export class InstancedThreatRenderer {
  ...
  private threatToIndex = new Map<string, { type: ThreatType; meshCategory: MeshCategory; index: number }>()
  ...
  addThreat(threat: Threat): boolean {
    ...
    if (!availableForType || availableForType.length === 0) {
      console.warn(`No available instance slots for threat type: ${type} (mesh category: ${meshCategory})`)
      return false
    }
