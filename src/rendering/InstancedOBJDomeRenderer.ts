...
export class InstancedOBJDomeRenderer {
  ...
  private async loadOBJModel(): Promise<void> {
    try {
      ...
      this.objInstancedMesh = new THREE.InstancedMesh(mergedGeometry, material, this.maxDomes);
  ...
  private createLauncherTubes(): void {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      matrix.makeTranslation(Math.cos(angle) * 0.8, 2.5, Math.sin(angle) * 0.8);
  ...
  private createProceduralFallback(): void {
    ...
    this.objInstancedMesh = new THREE.InstancedMesh(geometry, material, this.maxDomes);
    this.objInstancedMesh.castShadow = true;
    this.objInstancedMesh.receiveShadow = true;
