...
export class LODInstancedThreatRenderer extends InstancedThreatRenderer {
  ...
  private createLODMeshes() {
    ...
    Object.entries(threatLODs).forEach(([type, config]) => {
      ...
      const lodSet: LODMeshSet = {
        ...
        medium: new THREE.InstancedMesh(config.levels[1].geometry, material, this.maxThreatsPerType),
      ...
      this.lodAvailableIndices.set(`${baseKey}high`, Array.from({ length: this.maxThreatsPerType }, (_, i) => i))
      this.lodAvailableIndices.set(`${baseKey}medium`, Array.from({ length: this.maxThreatsPerType }, (_, i) => i))
      this.lodAvailableIndices.set(`${baseKey}low`, Array.from({ length: this.maxThreatsPerType }, (_, i) => i))
  ...
  private transferThreatLOD(threatId: string, category: MeshCategory, oldIndex: number, oldLOD: number, newLOD: number) {
    // Hide in old LOD
  ...
  private getLODMesh(category: MeshCategory, lodLevel: number): THREE.InstancedMesh {
    const lodSet = this.lodMeshes.get(category)!
    switch (lodLevel) {
      case 0: return lodSet.high
      case 1: return lodSet.medium
      case 2: return lodSet.low
      default: return lodSet.high
    }
  }
