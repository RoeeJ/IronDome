...
export class InterceptionSimulator {
  ...
  simulateInterception(scenario: TestScenario): SimulationResult {
    ...
    if (!interceptionSolution.shouldFire) {
      return {
        ...
        proximityDetonation: { detonated: false, detonationDistance: Infinity, detonationQuality: 0 },
        success: false
      }
    }
    ...
    for (let i = 0; i < guidanceResult.states.length; i++) {
      ...
      // Check detonation
      const detonationCheck = shouldDetonate(
        proximity,
  ...
  runScenarios(scenarios: TestScenario[]): {
    ...
    const avgHitDistance = successful.length > 0
      ? successful.reduce((sum, r) => sum + r.guidanceResult.hitDistance, 0) / successful.length
      : 0
    const avgHitTime = successful.length > 0
      ? successful.reduce((sum, r) => sum + r.guidanceResult.hitTime, 0) / successful.length
      : 0
    const avgDetonationQuality = successful.length > 0
      ? successful.reduce((sum, r) => sum + r.proximityDetonation.detonationQuality, 0) / successful.length
      : 0
