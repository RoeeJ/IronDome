...
export class ProximityFuseOptimizer {
  ...
  private createFitnessFunction(): (genome: Genome) => Promise<number> {
    return async (genome: Genome): Promise<number> => {
      ...
      for (const { scenario, weight } of scenarios) {
        ...
        const detonationDistance = result.proximityData.detonationDistance || settings.detonationRadius
        const efficiencyScore = 1 - Math.abs(detonationDistance - settings.optimalRadius) / settings.detonationRadius
   ...
   */
  async compareSettings(settingsArray: Array<{
    name: string
    settings: {
      armingDistance: number
      detonationRadius: number
    ...
    for (const { name, settings } of settingsArray) {
      ...
      const efficiency = performance.hitRate * performance.avgKillProbability / 
