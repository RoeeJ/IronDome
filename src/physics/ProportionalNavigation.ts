...
export class ProportionalNavigation {
  ...
  calculateGuidanceCommand(
    ...
    let commandAccel = omega.multiplyScalar(this.navigationConstant * closingVelocity)
  ...
  calculatePredictedMissDistance(
    ...
    const interceptorFinal = interceptorPos.clone()
      .add(interceptorVel.clone().multiplyScalar(timeToGo))
      .add(interceptorAccel.clone().multiplyScalar(0.5 * timeToGo * timeToGo))
    
    const targetFinal = targetPos.clone()
      .add(targetVel.clone().multiplyScalar(timeToGo))
  ...
  calculateOptimalLaunchAngle(
    ...
    const predictedPos = targetPos.clone()
      .add(targetVel.clone().multiplyScalar(timeToIntercept))
