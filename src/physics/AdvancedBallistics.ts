...
export class AdvancedBallistics {
  ...
  calculateTrajectory(
    ...
    // Total acceleration
    const totalForce = new THREE.Vector3(0, -gravity, 0).add(drag).add(coriolis).add(magnus);
  ...
  private calculateCoriolisForce(velocity: THREE.Vector3, position: THREE.Vector3): THREE.Vector3 {
  ...
  private calculateMagnusForce(velocity: THREE.Vector3, wind: THREE.Vector3): THREE.Vector3 {
  ...
  private calculateAirDensity(
    ...
    const altPressure =
      pressure * Math.pow(altTemp / tempK, -9.81 / (this.LAPSE_RATE * this.GAS_CONSTANT));
