...
export class DeveloperControls {
  ...
  private setupControls(): void {
      ...
      .add(
        {
          stressTest: () => {
            ...
            for (let i = 0; i < 50; i++) {
              setTimeout(() => {
                this.config.threatManager.spawnSpecificThreat(['rocket', 'mortar'][i % 2]);
              }, i * 100);
            }
      ...
      .add(
        {
          logThreatInfo: () => {
            ...
            threats.forEach((threat, i) => {
              debug.log(
                `Threat ${i}: Type=${threat.type}, Active=${threat.isActive}, ` +
    ...
    cleanupFolder
      ...
      .name('Force Cleanup Ground Effects');
    ...
    timeFolder.add(this.config.simulationControls, 'timeScale', 0.1, 5, 0.1).name('Time Scale');

    timeFolder.add(this.config.simulationControls, 'pause').name('Pause Simulation');
  }
  ...
  private toggleScreenshotMode(): void {
    ...
    this.config.showNotification(
      gameUI?.style.display === 'none' ? '📸 Screenshot mode ON' : 'Screenshot mode OFF'
    );
  }
