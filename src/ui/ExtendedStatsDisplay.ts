...
export class ExtendedStatsDisplay {
  ...
  update(): void {
    ...
    const successRate = stats.successful + stats.failed > 0
      ? ((stats.successful / (stats.successful + stats.failed)) * 100).toFixed(1)
      : '0.0'
    
    const interceptorEfficiency = stats.successful > 0
      ? (stats.successful / Math.max(1, stats.successful + stats.failed + stats.activeInterceptors)).toFixed(2)
      : '0.00'
