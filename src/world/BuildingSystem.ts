...
export class BuildingSystem {
  ...
  createBuilding(position: THREE.Vector3, width: number, height: number, depth: number): string {
    ...
    for (let row = startRow; row < windowRows; row++) {
      for (let col = 0; col < windowColsX; col++) {
        if (Math.random() < windowDensity) { // Uniform density
  ...
  private createBuildings(centerX: number, centerZ: number, radius: number): void {
    ...
    districts.forEach(district => {
      ...
      if (placedInDistrict >= 3) {
        ...
        console.warn(`Cancelling district at (${district.center.x}, ${district.center.z}) - only placed ${placedInDistrict} buildings`);
  ...
  private createFlatRoad(
    ...
    const mat = material ?? MaterialCache.getInstance().getMeshBasicMaterial({ 
      color: 0x444444,
  ...
  private createRoadNetwork(centerX: number, centerZ: number, radius: number): void {
    ...
    const neighborhoods: { center: { x: number; z: number }, buildings: typeof this.buildingPositions, radius: number }[] = [];
    ...
    for (const building of this.buildingPositions) {
      ...
      for (const candidate of candidates) {
        ...
        if (candidate.dist < neighborhoodRadius * 1.5) { // Slightly larger radius to ensure grouping
      ...
      for (const building of ungrouped) {
        ...
        for (const hood of neighborhoods) {
          const dist = Math.sqrt(
            Math.pow(building.x - hood.center.x, 2) + 
            Math.pow(building.z - hood.center.z, 2)
          );
        ...
        if (nearest) {
          ...
          // Recalculate center
          nearest.center.x = nearest.buildings.reduce((sum, b) => sum + b.x, 0) / nearest.buildings.length;
          nearest.center.z = nearest.buildings.reduce((sum, b) => sum + b.z, 0) / nearest.buildings.length;
        }
      }
    }
    ...
    neighborhoods.forEach(hood => {
      ...
      hood.buildings.forEach(building => {
        ...
        const buildingRadius = Math.sqrt(building.width * building.width + building.depth * building.depth) / 2;
    ...
    neighborhoods.forEach((hood, index) => {
      ...
      console.log(`Creating road for neighborhood at (${hood.center.x.toFixed(0)}, ${hood.center.z.toFixed(0)}), dist=${centerDist.toFixed(0)}, buildings=${hood.buildings.length}`);
    ...
    console.log(`Ring distribution: Inner=${innerRing.length}, Middle=${middleRing.length}, Outer=${outerRing.length}`);
    ...
    [innerRing, middleRing, outerRing].forEach((ring, ringIndex) => {
      ...
      const sortedRing = ring.map(hood => ({
        hood,
        angle: Math.atan2(hood.center.z, hood.center.x)
      })).sort((a, b) => a.angle - b.angle);
      
      console.log(`Sorted ${ringNames[ringIndex]} ring angles:`, sortedRing.map(s => (s.angle * 180 / Math.PI).toFixed(0)));
      ...
      sortedRing.forEach((current, index) => {
        ...
        console.log(`${ringNames[ringIndex]} ring: Checking connection ${index} -> ${(index + 1) % sortedRing.length}, dist=${dist.toFixed(0)}`);
    ...
    innerRing.forEach(innerHood => {
      ...
      if (closest && minDist < 400) { // Reasonable distance for inter-ring
        const angle = Math.atan2(
