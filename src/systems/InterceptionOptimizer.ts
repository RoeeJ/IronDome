...
export class InterceptionOptimizer {
   ...
   * Cache management
   */
  private getCachedSolution(threatId: string, batteryId: string): { point: THREE.Vector3; time: number } | null {
  ...
  private cacheSolution(
    ...
    if (this.calculationCache.size > 1000) {
      ...
      const sortedEntries = Array.from(this.calculationCache.entries())
  ...
  private updateMetrics(calculationTime: number, threatCount: number): void {
    ...
    if (this.calculationTimes.length >= 10) {
      const avgTime = this.calculationTimes.reduce((a, b) => a + b, 0) / this.calculationTimes.length
