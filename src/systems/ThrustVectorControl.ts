...
export class ThrustVectorControl {
  ...
  calculateThrustVector(
    ...
    const forwardDir = currentVelocity.length() > 5 ? velocityDir : desiredAcceleration.clone().normalize()
    
    // Calculate gimbal angles needed to achieve desired acceleration
    const gimbalCommand = this.calculateGimbalAngles(forwardDir, desiredAcceleration.clone().normalize(), dt)
    ...
    const thrustVector = this.applyGimbalToThrust(forwardDir, this.gimbalAngle)
   ...
   */
  private applyGimbalToThrust(
    nominalDir: THREE.Vector3,
    gimbal: THREE.Vector2
  ): THREE.Vector3 {
  ...
  private calculateDACSAssist(
    ...
    const lateralAccel = desiredAccel.clone()
      .sub(mainThrust.clone().normalize().multiplyScalar(desiredAccel.dot(mainThrust) / thrustMag))
    ...
    const dacsThrust = lateralAccel.normalize().multiplyScalar(
      Math.min(this.config.dacsThrustPulse, lateralMag * 10)
    )
  ...
  applyForces(
    ...
    if (dacsCommand.length() > 0 && this.dacsRemainingImpulse > 0) {
      const dacsForce = new CANNON.Vec3(
        dacsCommand.x,
        dacsCommand.y,
        dacsCommand.z
      )
      ...
      debug.category('TVC', `DACS pulse: ${dacsCommand.length().toFixed(1)}N, remaining: ${this.dacsRemainingImpulse.toFixed(0)}Ns`)
    }
  }
