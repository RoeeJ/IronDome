...
export class InterceptorAllocation {
  ...
  optimizeAllocation(
    ...
    debug.module('InterceptorAllocation').log(`Optimizing allocation for ${threats.length} threats and ${batteries.length} batteries`)
    
    // Step 1: Analyze all threats
    const threatMetrics = threats.map(threat => this.analyzeThreat(threat))
      .sort((a, b) => b.priority - a.priority) // Sort by priority
    
    // Step 2: Assess battery capabilities
    const batteryCapabilities = this.assessBatteries(batteries, threats)
    
    debug.module('InterceptorAllocation').log(`Battery capabilities:`, batteryCapabilities.map(cap => ({
    ...
    debug.module('InterceptorAllocation').log(`Allocation complete: ${allocations.size} allocated, ${unassignedThreats.length} unassigned`)
  ...
  private analyzeThreat(threat: Threat): ThreatMetrics {
    ...
    debug.module('InterceptorAllocation').log(`Threat ${threat.id} analysis: priority=${priority}, difficulty=${difficulty.toFixed(2)}, successRate=${successRate}, required=${requiredInterceptors}`)
    ...
    for (const threatMetric of threats) {
      ...
      for (const capability of capabilities) {
        ...
        if (available < threatMetric.requiredInterceptors) {
          skippedReasons.push(`Battery has ${available} interceptors, needs ${threatMetric.requiredInterceptors}`)
          continue
        }
        
        // Score this allocation
        const score = this.scoreAllocation(
          threatMetric,
          capability,
          available,
          threats.length
      ...
      if (bestAllocation) {
        ...
        debug.module('InterceptorAllocation').log(`Could not allocate threat ${threatMetric.threat.id}: ${skippedReasons.join(', ')}`)
      }
    }
