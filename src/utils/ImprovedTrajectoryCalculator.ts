...
export class ImprovedTrajectoryCalculator {
  ...
  static calculateInterceptionPoint(
    ...
    if (isDrone) {
      return this.calculateDroneInterception(threatPos, threatVel, interceptorPos, interceptorSpeed)
    }
    
    // For ballistic threats, use improved numerical method
    return this.calculateBallisticInterception(threatPos, threatVel, interceptorPos, interceptorSpeed)
  }
  ...
  private static calculateDroneInterception(
    ...
    if (Math.abs(threatSpeed - s) < 0.1) {
      ...
      return this.calculateDroneInterceptionIterative(threatPos, threatVel, interceptorPos, interceptorSpeed)
    }
  ...
  private static getInitialTimeEstimate(
    ...
    const approachSpeed = -threatVel.clone().normalize().dot(
      interceptorPos.clone().sub(threatPos).normalize()
    ) * threatVel.length()
  ...
  private static findGroundHitTime(threatPos: THREE.Vector3, threatVel: THREE.Vector3): number | null {
  ...
  static calculateMultipleInterceptionWindows(
    ...
    // First interception
    const first = this.calculateInterceptionPoint(threatPos, threatVel, interceptorPos, interceptorSpeed, false)
    ...
    if (secondPos.y > 100) { // Minimum altitude for second attempt
