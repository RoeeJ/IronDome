...
export class ThreatTracker {
  ...
  startTracking(threat: Threat): void {
    ...
    kalmanFilter.initializeFromThreat(threat.getPosition(), threat.getVelocity(), threat.type);
  ...
  getTrackedState(
    threatId: string
  ): { position: THREE.Vector3; velocity: THREE.Vector3; acceleration: THREE.Vector3 } | null {
    const tracked = this.trackedThreats.get(threatId);
    if (!tracked) return null;
