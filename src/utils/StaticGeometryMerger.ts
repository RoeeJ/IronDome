...
export class StaticGeometryMerger {
  ...
  private static mergeMultiMaterial(materialGroups: Map<THREE.Material, THREE.Mesh[]>): THREE.Mesh {
    ...
    materialGroups.forEach((meshes, material) => {
      ...
      const groups = mergedGeometry.groups.length > 0 ? mergedGeometry.groups : [{
        start: 0,
        count: mergedGeometry.index ? mergedGeometry.index.count : 
  ...
  static mergeStaticCity(buildingMeshes: THREE.Mesh[], roadMeshes: THREE.Mesh[], lightMeshes: THREE.Mesh[]): {
