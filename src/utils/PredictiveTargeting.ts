...
export class PredictiveTargeting {
  private threatHistory: Map<
    string,
    Array<{ position: THREE.Vector3; velocity: THREE.Vector3; time: number }>
  > = new Map();
  ...
  predictTrajectory(
    ...
    const currentPos = threat.getPosition().clone();
    const currentVel = threat.getVelocity().clone();
