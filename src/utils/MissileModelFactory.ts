...
export class MissileModelFactory {
  ...
  private createRocketModel(
    ...
    // Warhead (nose cone)
    const warheadGeometry = this.geometryFactory.getCone(bodyRadius * 0.8, bodyLength * 0.3, 8);
    ...
    // Tail fins
    const finGeometry = this.geometryFactory.getBox(bodyRadius * 2, 0.05, bodyRadius * 0.8);
    const finMaterial = this.materialCache.getMeshStandardMaterial({
      color: 0x555555,
      roughness: 0.7,
      metalness: 0.3,
  ...
  private createMortarModel(config: ThreatConfig): THREE.Object3D {
    ...
    const finGeometry = this.geometryFactory.getBox(config.radius * 1.2, 0.03, config.radius * 0.4);
    const finMaterial = this.materialCache.getMeshStandardMaterial({
      color: 0x444444,
      roughness: 0.8,
      metalness: 0.2,
  ...
  private createCruiseMissileModel(config: ThreatConfig): THREE.Object3D {
    ...
    // Nose cone - sharper
    const noseGeometry = this.geometryFactory.getCone(config.radius * 0.7, config.radius * 1.8, 8);
    const noseMaterial = this.materialCache.getMeshStandardMaterial({
      color: 0xaaaaaa,
      roughness: 0.2,
      metalness: 0.9,
    ...
    // Wings - swept back
    const wingGeometry = this.geometryFactory.getBox(config.radius * 3, 0.1, config.radius * 1.2);
