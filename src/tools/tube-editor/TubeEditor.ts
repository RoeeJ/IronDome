...
export class TubeEditor {
  ...
  private onMouseClick(event: MouseEvent): void {
    ...
    if (intersects.length > 0) {
      ...
      if (this.isPlacingStart || !tube.start) {
        ...
        // Calculate direction
        tube.direction = new THREE.Vector3().subVectors(tube.end, tube.start).normalize();
  ...
  private initializeGridControls(): void {
    ...
    selectPointsBtn.addEventListener('click', () => {
      ...
      if (this.selectionMode === 'corners') {
        cornerStatus.innerHTML =
          'Click corners in order: <span style="color:#ff0000">1. Top-Left</span>, <span style="color:#00ff00">2. Top-Right</span>, <span style="color:#0000ff">3. Bottom-Right</span>, <span style="color:#ffff00">4. Bottom-Left</span>';
      } else if (this.selectionMode === 'centers') {
        cornerStatus.innerHTML =
          'Click center tubes in order: <span style="color:#ff0000">1. Row1,Col1</span>, <span style="color:#00ff00">2. Row1,Col5</span>, <span style="color:#0000ff">3. Row4,Col5</span>, <span style="color:#ffff00">4. Row4,Col1</span>';
      } else {
        cornerStatus.innerHTML =
          'Select 4 corners of <span style="color:#ff0000">Tube 1 (Row1,Col1)</span> - Corner 1/4';
      }
    ...
    selectionModeSelect.addEventListener('change', e => {
      this.selectionMode = (e.target as HTMLSelectElement).value as
        | 'corners'
        | 'centers'
        | 'tube-corners';
    });
  ...
  private applyGridNormalization(): void {
    ...
    const rowSpacing = parseFloat(
      (document.getElementById('row-spacing') as HTMLInputElement).value
    );
    const colSpacing = parseFloat(
      (document.getElementById('col-spacing') as HTMLInputElement).value
    );
    const tubeLength = parseFloat(
      (document.getElementById('tube-length') as HTMLInputElement).value
    );
    ...
    if (this.selectionMode === 'centers' || this.selectionMode === 'tube-corners') {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          const endPoint = startPoint.clone().add(avgDirection.clone().multiplyScalar(tubeLength));
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          this.visualizer.updateTubeLabel(
            tubeIndex,
            `${tubeIndex + 1} (R${actualRow + 1}C${col + 1})`
          );
