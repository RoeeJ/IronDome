...
describe('Simple GA Optimization Test', () => {
  test('optimize simple quadratic function', async () => {
    ...
    expect(result.bestGenome.fitness).toBeCloseTo(await fitnessFunction(result.bestGenome), 10);

    console.log('\n=== RESULTS ===');
    console.log('Target: (5, 3)');
    console.log(
      `Found: (${result.bestGenome.genes.x.toFixed(2)}, ${result.bestGenome.genes.y.toFixed(2)})`
    );
