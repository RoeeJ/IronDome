...
describe('Trajectory Systems Tests', () => {
  describe('TrajectoryCalculator', () => {
    describe('calculateLaunchParameters', () => {
      ...
      it('should calculate launch parameters for lofted trajectory', () => {
        ...
        const paramsLofted = TrajectoryCalculator.calculateLaunchParameters(
          position,
          target,
          velocity,
          true
        );
        const paramsDirect = TrajectoryCalculator.calculateLaunchParameters(
          position,
          target,
          velocity,
          false
  ...
  describe('PredictiveTargeting', () => {
    ...
    describe('calculateLeadPrediction', () => {
      it('should predict lead for moving targets', () => {
        ...
        const prediction = targeting.calculateLeadPrediction(threat, batteryPos, interceptorSpeed);
  ...
  describe('ProportionalNavigation (Unit Tests)', () => {
    ...
    it('should calculate PN guidance commands', () => {
      ...
      // Independent planar true-PN components: range²=42500, -r·vr=30500.
      const gain = (3 * 30500) / 42500;
      expect(command.acceleration.x).toBeCloseTo(gain * (-150 + (200 * 30500) / 42500), 10);
      expect(command.acceleration.y).toBeCloseTo(gain * (-10 + (50 * 30500) / 42500), 10);
      expect(command.acceleration.z).toBe(0);
      expect(command.acceleration.length()).toBeLessThanOrEqual(300);
    });
  ...
  describe('AdvancedBallistics (Unit Tests)', () => {
    ...
    it('should calculate trajectory with environmental factors', () => {
      ...
      const environmental = {
        ...
        airDensity: 1.225,
        temperature: 20,
        pressure: 1013.25, // API takes local hPa, not Pa
        humidity: 0.5,
        altitude: 0,
      };
